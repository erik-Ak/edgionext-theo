...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (
      ...
      req.query.token !== "5MbOHiS.b2KrIT.8nQ9HsYbm"
    ) {
      res.status(403).send("");
    }
    ...
    if (elasticRes && elasticRes.errors === false) {
      ...
      const errorSlackRes = await axios.post(
        "https://hooks.slack.com/services/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",
        {
          text:
            "Error in Elasticsearch " +
            JSON.stringify(elasticRes ? elasticRes : []),
        }
      );
